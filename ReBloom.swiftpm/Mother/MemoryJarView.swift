...
struct MemoryJarView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(isPresented: $showAddSheet) {
                AddMemorySheet(partnerName: profile?.partnerName ?? "Partner") { memory in
    ...
    private var introBanner: some View {
        let partnerName = profile?.partnerName ?? "your partner"
            ...
            ForEach(sortedKeys, id: \.self) { dateKey in
                if let dayMemories = grouped[dateKey] {
                    VStack(alignment: .leading, spacing: 14) {
                        // Date pill header — aligned with nav title (20pt inset)
                        HStack(spacing: 8) {
                            Text(dateKey)
    ...
    var body: some View {
        Button {
            ...
            ZStack(alignment: .bottomLeading) {
                // Photo layer — clipped inside the ZStack
                    ...
                    if memory.isSharedWithPartner {
                        HStack(spacing: 3) {
                            Image(systemName: "heart.fill").font(.system(size: 9))
                                .foregroundStyle(memory.sharedBy == "partner" ? Color.partnerPrimary : Color.motherRose)
            ...
            ZStack {
                ...
                if showBars {
                    VStack {
                        ...
                        .frame(maxWidth: .infinity, alignment: .leading)
                        .padding(20)
                        .padding(.bottom, 50) // space for bottom toolbar
                        .background(
                            LinearGradient(
