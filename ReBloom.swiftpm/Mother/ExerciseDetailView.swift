...
struct ExerciseDetailView: View {
    ...
    @State private var exerciseState: ExerciseState = .notStarted
    ...
    private func startTimer() {
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 1, repeats: true) { _ in
            seconds += 1
            if let duration = selectedDuration, seconds >= duration { completeExercise() }
        }
    }
    ...
    private func endExerciseFully() {
        timer?.invalidate(); timer = nil
        // Log as completed when user manually ends
        let log = ExerciseLog(
            exerciseName: exercise.name,
            weekNumber: profile?.weeksPostpartum ?? 1,
            completed: true
