...
struct JournalView: View {
        ...
        MotherGlassCard {
            VStack(spacing: 16) {
                ...
                ZStack(alignment: .topLeading) {
                    if vm.entryText.isEmpty && !isTextFieldFocused {
                    ...
                    TextEditor(text: $vm.entryText)
                        ...
                        .fontDesign(.rounded)
                        .foregroundStyle(Color.motherTextHeading)
                        .onChange(of: vm.entryText) { _, newValue in
                            if newValue.count > 500 {
                                vm.entryText = String(newValue.prefix(500))
                            }
                        }
                }
                .background(
                ...
                HStack {
                    Spacer()
                    Text("\(vm.entryText.count)/500")
                        .font(.caption2)
                        .fontDesign(.rounded)
                        .foregroundStyle(vm.entryText.count > 450 ? Color.motherRose : Color.secondary.opacity(0.5))
                }
                ...
                VStack(spacing: 12) {
                    ...
                    .buttonStyle(.plain)
                    .disabled(vm.entryText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
                    .opacity(vm.entryText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? 0.5 : 1.0)
                    ...
                    Button {
                        ...
                        HStack(spacing: 10) {
                            Image(systemName: "heart.text.square.fill")
                                .font(.system(size: 16, weight: .semibold))
                            Text("Share with \(vm.partnerName)")
                                .font(.body.weight(.semibold))
                                .fontDesign(.rounded)
                        }
                        .foregroundStyle(.white)
    ...
    private var voiceRecordingCard: some View {
        MotherGlassCard {
            VStack(spacing: 16) {
                ...
                Text("Record and share your voice with \(vm.partnerName)")
                    .font(.subheadline)
                    .fontDesign(.rounded)
                    .foregroundStyle(Color.motherTextBody)
                    .frame(maxWidth: .infinity, alignment: .leading)
    ...
    private var pastEntriesSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(spacing: 12) {
                ForEach(vm.journalEntries, id: \.id) { entry in
