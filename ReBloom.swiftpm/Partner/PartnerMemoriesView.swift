...
struct PartnerMemoriesView: View {
    ...
    private var memories: [Memory] {
        allMemories.filter { $0.sharedBy == "partner" || $0.isSharedWithPartner }
    }
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(isPresented: $showAddSheet) {
                PartnerAddMemorySheet(partnerName: profile?.partnerName ?? "Mom") { memory in
            ...
            .fullScreenCover(item: $selectedMemory) { memory in
                PartnerMemoryDetailView(
                    memory: memory,
                    motherName: profile?.partnerName ?? "Mom"
                )
            }
    ...
    private var introBanner: some View {
        let motherName = profile?.partnerName ?? "your partner"
    ...
    private var memoriesGrid: some View {
        let grouped = groupedByDate(memories)
    ...
    private func groupedByDate(_ items: [Memory]) -> [String: [Memory]] {
                    ...
                    if memory.isSharedWithPartner {
                        HStack(spacing: 3) {
                            Image(systemName: "heart.fill").font(.system(size: 9))
                                .foregroundStyle(memory.sharedBy == "mother" ? Color.motherRose : Color.partnerPrimary)
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            if memory.isSharedWithPartner {
                                HStack(spacing: 5) {
                                    Image(systemName: "heart.fill")
                                        .font(.system(size: 11))
                                    Text(memory.sharedBy == "mother" ? "Shared by \(motherName)" : "Shared with \(motherName)")
                                        .font(.caption.weight(.semibold))
                                        .fontDesign(.rounded)
                                }
