...
struct HealView: View {
    ...
    @State private var vm = HealViewModel()
    ...
    private var weekBanner: some View {
        VStack(spacing: 10) {
            Text("Week \(vm.currentWeekNumber)")
    ...
    private var weekDayStrip: some View {
        let weekStartOffset = ((vm.currentWeekNumber - 1) * 7)

        return HStack(spacing: 8) {
            ForEach(0..<7, id: \.self) { index in
                let relativeDayIndex = weekStartOffset + index
                let dayNumber = index + 1
                let isToday     = dayNumber == vm.currentDayInWeek
                let isCompleted = vm.completedRelativeDays.contains(relativeDayIndex)
                let isFuture    = dayNumber > vm.currentDayInWeek
